...
public class NetvirtVpnUtils {
    ...
    private static Adjacencies buildInterfaceAdjacency(String ipAddress, MacAddress macAddress, boolean primary, String nextHopIp) {       
    ...
    public static void createVpnPortFixedIp(DataBroker dataBroker, String vpnName,  String portName, IpPrefix ipAddress, MacAddress macAddress) {
    ...
    public static void createVpnPortFixedIp(DataBroker dataBroker, String vpnName,  String portName, IpAddress ipAddress, MacAddress macAddress) {
        String fixedIp =  MefUtils.ipAddressToString(ipAddress);
        createVpnPortFixedIp(dataBroker, vpnName, portName, fixedIp, macAddress);
    }

    public static void createVpnPortFixedIp(DataBroker dataBroker, String vpnName,  String portName, String fixedIp, MacAddress macAddress) {     
        synchronized ((vpnName + fixedIp).intern()) {
            ...
            VpnPortipToPortBuilder builder = new VpnPortipToPortBuilder().setKey(
                    ...
                    (portName).setMacAddress(macAddress.getValue()).setSubnetIp(true).setConfig(true)
                    .setLearnt(false);
            MDSALUtil.syncWrite(dataBroker, LogicalDatastoreType.OPERATIONAL, id, builder.build());
            logger.debug("Interface to fixedIp added: {}, vpn {}, interface {}, mac {} added to " +
    ...
    public static void addDirectSubnetToVpn(DataBroker dataBroker, final NotificationPublishService notificationPublishService,  
            String vpnName, String subnetName, IpPrefix subnetIpPrefix, String interfaceName) {
        InstanceIdentifier<ElanInstance> elanIdentifierId =
                InstanceIdentifier.builder(ElanInstances.class).child(ElanInstance.class,
                        new ElanInstanceKey(subnetName)).build();
    ...
    public static MacAddress resolveGwMac(DataBroker dataBroker, OdlArputilService arpUtilService, 
            String vpnName, IpPrefix srcIpPrefix, IpAddress dstIpAddress, String interf) {
    ...
    private static void sendArpRequest (OdlArputilService arpUtilService,  IpAddress srcIpAddress, IpAddress dstIpAddress, String interf) {
        try {
            List<InterfaceAddress> interfaceAddresses = new ArrayList<>();
            interfaceAddresses.add(new InterfaceAddressBuilder().setInterface(interf).setIpAddress(srcIpAddress).build());
    ...
    public static MacAddress waitForArpReplyProcessing(DataBroker dataBroker, String vpnName,  IpAddress dstIpAddress, int retries) {
        while (retries > 0) {
            InstanceIdentifier<VpnPortipToPort> optionalPortIpId = buildVpnPortipToPortIdentifier(vpnName, MefUtils.ipAddressToString(dstIpAddress));
            Optional<VpnPortipToPort> optionalPortIp = MdsalUtils.read(dataBroker, LogicalDatastoreType.OPERATIONAL, optionalPortIpId);
