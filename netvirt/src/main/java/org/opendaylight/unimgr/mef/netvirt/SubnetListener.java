...
public class SubnetListener extends UnimgrDataTreeChangeListener<Subnet> {
    ...
    private void createNetwork(DataTreeModification<Subnet> newDataObject) {
        ...
        Identifier45 subUniId = newSubnet.getUniId();
        Identifier45 subIpUniId = newSubnet.getIpUniId();

        InstanceIdentifier<Ipvc> ipvcId = findService(subUniId, subIpUniId);
        if (ipvcId == null) {
            Log.info("Subnet Uni {} IpUNI {} is not assosiated to service", subUniId, subIpUniId);
            return;
        }
        IpvcVpn ipvcVpn = MefServicesUtils.getOperIpvcVpn(dataBroker, ipvcId);
        if (ipvcVpn == null || ipvcVpn.getVpnElans() == null) {
            Log.error("Subnet Uni {} IpUNI {} is not operational", subUniId, subIpUniId);
            return;
        }
        checkCreateDirectNetwork(subUniId, subIpUniId, newSubnet, ipvcVpn);

    }

    private void checkCreateDirectNetwork(Identifier45 subUniId, Identifier45 subIpUniId, Subnet newSubnet,
            IpvcVpn ipvcVpn) {
        ...
        for (VpnElans vpnElan : vpnElans) {
            if (vpnElan.getUniId().equals(newSubnet.getUniId())
                ...
                if (vpnElan.getSubnets() != null && vpnElan.getSubnets().contains(newSubnet)) {
                    Log.info("Network {} already created under UNI {} IpUni {}", newSubnet, subUniId, subIpUniId);
                    return;
                }
    ...
    private void createNonConnectedNetwork() {
        /*
