...
public class VbrHandler extends UnimgrDataTreeChangeListener<Subscribers> {
    ...
    public void add(DataTreeModification<Subscribers> newDataObject) {
        ...
        MatchIpv4Source upstreamIpMatch = new MatchIpv4Source(new Ipv4Prefix(ip.getValue()));
        ...
        upstreamActionsInfos.add(new ActionPushVlan(sessionId));
        ...
        FlowEntity upstreamFlow = MDSALUtil.buildFlowEntity(new BigInteger(dpnID), (short)0,
                ip.getValue(),
                1000, ip.getValue() ,0,0,BigInteger.ZERO,upstreamMatches, upstreamInstructions);
        mdsalUtil.installFlow(upstreamFlow);

        List<MatchInfo> downstreamMatches = new ArrayList<>();
        MatchIpv4Destination downstreamIpMatch = new MatchIpv4Destination(new Ipv4Prefix(ip.getValue()));
        ...
        downstreamInstructions.add(new InstructionApplyActions(upstreamActionsInfos));

        FlowEntity downstreamFlow = MDSALUtil.buildFlowEntity(new BigInteger(dpnID), (short)0,
                ip.getValue(),
                1000, ip.getValue() ,0,0,BigInteger.ZERO,downstreamMatches, downstreamInstructions);
