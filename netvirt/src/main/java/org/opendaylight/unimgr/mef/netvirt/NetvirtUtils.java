...
public class NetvirtUtils {
    ...
    private static String convertNetworkToNetvirtNetwork(DataBroker dataBroker, String unimgrNetworkId,
        ...
        Optional<MefService> optionalMefService = MefServicesUtils.getOpMefServiceBySvcId(dataBroker, svcId);
        if (!optionalMefService.isPresent()) {
            logger.warn("no mef-service found for svc-id {}", svcId);
            return null;
        }
        MefService mefService = optionalMefService.get();
        String networkId = null;
        if (mefService.getMefServiceChoice() instanceof EvcChoice) {
            networkId = ((EvcChoice) mefService.getMefServiceChoice()).getEvc().getAugmentation(EvcElan.class)
                    .getElanId();
        } else if (mefService.getMefServiceChoice() instanceof IpvcChoice) {
            List<VpnElans> vpnElansList = ((IpvcChoice) mefService.getMefServiceChoice()).getIpvc()
            ...
            networkId = (vpnElan != null) ? vpnElan.getElanId() : null;
        }
        return networkId;
    }
