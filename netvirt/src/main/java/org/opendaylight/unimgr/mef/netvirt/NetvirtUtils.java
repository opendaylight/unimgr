...
public class NetvirtUtils {
    ...
    public static Interface getIetfInterface(DataBroker dataBroker, String interfaceName) {
        Optional<Interface> optional = MdsalUtils.read(dataBroker, LogicalDatastoreType.CONFIGURATION,
                getInterfaceIdentifier(interfaceName));
    ...
    public static Integer getIetfVlanData(DataBroker dataBroker, String interfaceName) {
        IfL2vlan augmentation = getIetfInterface(dataBroker, interfaceName).getAugmentation(IfL2vlan.class);
        return augmentation.getVlanId().getValue();
    }
}
