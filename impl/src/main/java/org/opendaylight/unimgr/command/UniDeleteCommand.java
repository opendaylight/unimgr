...
public class UniDeleteCommand extends AbstractDeleteCommand {
    ...
    public void execute() {
        ...
        if (!removedUnis.isEmpty()) {
            LOG.info("removedUnis not empty.");
            for (InstanceIdentifier<UniAugmentation> removedUniIid: removedUnis) {
                ...
                if(optUni.isPresent()){
                    ...
                    Optional<Node> optNode = UnimgrUtils.readNode(dataBroker, iidNode);
        ...
        if(!removedNodes.isEmpty()) {
            LOG.info("removedNodes not empty.");
    ...
    private void deleteNode(InstanceIdentifier<Node> iidNode) {
        ...
        try {
            ...
            LOG.warn("Failed to delete iidNode {}", e.getMessage());
        }
    }
