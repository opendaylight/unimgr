...
public class EvcDataTreeChangeListener implements IUnimgrDataTreeChangeListener<Link> {
    ...
    public void onDataTreeChanged(Collection<DataTreeModification<Link>> collection) {
        for (DataTreeModification<Link> change : collection) {
             ...
             LOG.info("Config for evc link {} and modification type is {}", rootLink.getIdentifier(), rootLink.getModificationType());
             //Map<InstanceIdentifier<?>, DataObject> changes = new HashMap();
             //changes.put(change.getRootPath().getRootIdentifier(), rootNode.getDataAfter());
             switch (rootLink.getModificationType()) {
                 case SUBTREE_MODIFIED:
                     LOG.info("Config for evc link {} updated", rootLink.getIdentifier());

                     break;
                 case WRITE:
                     LOG.info("Config for evc link {} created", rootLink.getIdentifier());

                     break;
                 case DELETE:
                     LOG.info("Config for evc link {} deleted", rootLink.getIdentifier());
