...
public class UnimgrUtils {
    ...
    public static Node createQoSForOvsdbNode (DataBroker dataBroker, UniAugmentation uni) {
        ...
        if (optionalNode.isPresent()) {
            ...
            InstanceIdentifier<OvsdbNodeAugmentation> ovsdbNodeAugmentationIid = UnimgrMapper.getOvsdbNodeIid(ovsdbNodeId)
            ...
            try {
                future.checkedGet();
                LOG.info("Update qos and queues to ovsdb for node {} {}", ovsdbNodeId, ovsdbNodeAugmentationIid);
    ...
    public static void updateQosEntries(DataBroker dataBroker, UniAugmentation uni) {
        Optional<Node> optionalNode = findOvsdbNode(dataBroker, uni);
        if (optionalNode.isPresent()) {
            NodeId ovsdbNodeId = optionalNode.get().getNodeId();
            Long queueNumber = 0L;
            List<QosEntries> qosList = optionalNode.get().getAugmentation(OvsdbNodeAugmentation.class).getQosEntries();
            LOG.info("QOS entries list {} for node {}", qosList, ovsdbNodeId);
            ...
            Uuid queueUuid = null;
            List<Queues> queuesList = optionalNode.get().getAugmentation(OvsdbNodeAugmentation.class).getQueues();
