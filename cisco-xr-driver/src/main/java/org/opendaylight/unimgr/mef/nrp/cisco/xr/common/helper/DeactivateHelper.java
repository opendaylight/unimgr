...
/*
			...
			*/
			...
			if (XRVersion.equals(NetconfConstants.XR_VERSION_SIX_SIX)) {
				if (!ServicePort.isSameDevice(endpoint, dvls)) {
					transaction.delete(LogicalDatastoreType.CONFIGURATION,
							L2vpnXrVersionSixDotSixHelper.getXConnectIds(innerOuterName, innerOuterName));
				}
				transaction.delete(LogicalDatastoreType.CONFIGURATION,
						L2vpnXrVersionSixDotSixHelper.getInterfaceConfigurationId(port, isExclusive));
			}
			...
			*/
			...
			if(XRVersion.equals(NetconfConstants.XR_VERSION_SIX_SIX)) { 
				if (!AbstractL2vpnBridgeDomainActivator.isSameInterface(endpoint, inls)) {
					if (!ServicePort.isSameDevice(endpoint, dvls)) {
						transaction.delete(LogicalDatastoreType.CONFIGURATION,
								L2vpnXrVersionSixDotSixHelper.getBridgeDomainIds(innerOuterName, innerOuterName));
					}
					transaction.delete(LogicalDatastoreType.CONFIGURATION,
							L2vpnXrVersionSixDotSixHelper.getInterfaceConfigurationId(port, isExclusive));
				}
