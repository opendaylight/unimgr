...
import org.opendaylight.unimgr.mef.nrp.cisco.xr.version.six.five.l2vpn.helper.XConnectHelper;
import org.opendaylight.unimgr.mef.nrp.cisco.xr.version.six.one.l2vpn.helper.DeactivateHelper;
...
*/
    ...
    /*protected void deactivate(ServicePort port, InstanceIdentifier<?> ids,
        ...
        if (ids.getTargetType().equals(
            ...
            transaction.delete(LogicalDatastoreType.CONFIGURATION, interfaceConfigurationId);
        } else if(ids.getTargetType().equals(
    ...
    protected void deactivateBridgeDomain(ServicePort port,
            InstanceIdentifier<InterfaceConfiguration> interfaceConfigurationId, String serviceId, boolean isExclusive,
            EndPoint endpoint, MountPointService mountService, List<String> dvls, List<Uuid> inls)
            throws InterruptedException, ExecutionException {
    	LOG.debug("********************************IN Deactivate Bridge Domain******************************");
        Optional<DataBroker> optional = MountPointHelper.getDataBroker(mountService, port.getNode().getValue());
        if (!optional.isPresent()) {
            LOG.error("Could not retrieve MountPoint for {}", port.getNode().getValue());
            return;
        }
        LOG.debug("********************************IN Deactivate Bridge Domain******************************1");
        WriteTransaction transaction = optional.get().newWriteOnlyTransaction();
        LOG.debug("********************************IN Deactivate Bridge Domain******************************2");
        String innerOuterName = new FixedServiceNaming().replaceForbidenCharacters(serviceId);
        LOG.debug("********************************IN Deactivate Bridge Domain******************************3"+innerOuterName);
        ...
        LOG.debug("*******************************After Running DeactivateHelper********************"); 
        transaction.commit().get();
        LOG.debug("*******************************After Running Transectctioins Commit *****************************");
    }
   
      
    protected void deactivateXConnect(ServicePort port, 
            InstanceIdentifier<InterfaceConfiguration> interfaceConfigurationId, String serviceId, boolean isExclusive,
            EndPoint endpoint, MountPointService mountService, List<String> dvls, List<Uuid> inls)
            throws InterruptedException, ExecutionException {
    	LOG.debug("********************************IN Deactivate Xconnect Method ******************************");
        Optional<DataBroker> optional = MountPointHelper.getDataBroker(mountService, port.getNode().getValue());
        if (!optional.isPresent()) {
            LOG.error("Could not retrieve MountPoint for {}", port.getNode().getValue());
            return;
        }
        LOG.debug("********************************After getting options object ******************************");
        WriteTransaction transaction = optional.get().newWriteOnlyTransaction();
        LOG.debug("********************************After getting options object ******************************1");
        String innerOuterName = new FixedServiceNaming().replaceForbidenCharacters(serviceId);
        LOG.debug("********************************After getting options object ******************************2"+innerOuterName);
      //  DeactivateHelper.deativateBridgeDomainConfiguration(port,interfaceConfigurationId,innerOuterName,endpoint,dvls,inls,transaction);
        DeactivateHelper.deativateXconnectConfiguration(port,interfaceConfigurationId,innerOuterName,endpoint,dvls,transaction);
        LOG.debug("********************************After getting options object ******************************3");
        transaction.commit().get();
        LOG.debug("*******************************After Running Transectctioins Commit *****************************");
    }                                                                                                                              
}
