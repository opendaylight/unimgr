...
//import org.opendaylight.unimgr.mef.nrp.cisco.xr.version.six.five.InterfaceXrVersionSixDotFiveHelper;
//import org.opendaylight.unimgr.mef.nrp.cisco.xr.version.six.five.L2vpnXrVersionSixDotFiveHelper;
//import org.opendaylight.unimgr.mef.nrp.cisco.xr.version.six.one.InterfaceXrVersionSixDotOneHelper;
//import org.opendaylight.unimgr.mef.nrp.cisco.xr.version.six.one.L2vpnXrVersionSixDotOneHelper;
import org.opendaylight.unimgr.mef.nrp.cisco.xr.version.six.six.InterfaceXrVersionSixDotSixHelper;
import org.opendaylight.unimgr.mef.nrp.cisco.xr.version.six.six.L2vpnXrVersionSixDotSixHelper;
import org.opendaylight.unimgr.mef.nrp.cisco.xr.version.six.three.InterfaceXrVersionSixDotThreeHelper;
        ...
        return nodes.stream().flatMap(cn -> {
            ...
            try {
                ...
                if (mountPoint.isPresent()) {
                      ...
                      /*
                      try {
                          if(InterfaceXrVersionSixDotOneHelper.ports(tx).count() > 0 
                              && L2vpnXrVersionSixDotOneHelper.checkL2vpnCapability(netconf)) {
                              nodeXRVersionMap.put(key.getNodeId(), NetconfConstants.XR_VERSION_SIX_ONE);
                              return InterfaceXrVersionSixDotOneHelper.toTp(nodes, mountService, mountIds).stream();
                            }
                        }
                    catch (Exception e) {
                      ...
                      */
                      ...
                      try {
                            if(InterfaceXrVersionSixDotThreeHelper.ports(tx).count() > 0 
                                && L2vpnXrVersionSixDotThreeHelper.checkL2vpnCapability(netconf)) {
                                nodeXRVersionMap.put(key.getNodeId(), NetconfConstants.XR_VERSION_SIX_THREE);
                                return InterfaceXrVersionSixDotThreeHelper.toTp(nodes, mountService, mountIds).stream();
                              }
                          }
                      catch (Exception e) {
                            // TODO Auto-generated catch block
					...
					/*
					 * try { if (InterfaceXrVersionSixDotFiveHelper.ports(tx).count() > 0 &&
					 * L2vpnXrVersionSixDotFiveHelper.checkL2vpnCapability(netconf)) {
					 * nodeXRVersionMap.put(key.getNodeId(), NetconfConstants.XR_VERSION_SIX_FIVE);
					 * return InterfaceXrVersionSixDotFiveHelper.toTp(nodes, mountService,
					 * mountIds).stream(); } } catch (Exception e) { // TODO Auto-generated catch
					 * block }
					 */
					
                      try {
                          if(InterfaceXrVersionSixDotSixHelper.ports(tx).count() > 0 
                              && L2vpnXrVersionSixDotSixHelper.checkL2vpnCapability(netconf)) {
                              nodeXRVersionMap.put(key.getNodeId(), NetconfConstants.XR_VERSION_SIX_SIX);
                              return InterfaceXrVersionSixDotSixHelper.toTp(nodes, mountService, mountIds).stream();
                            }
                        }
                    catch (Exception e) {
                          // TODO Auto-generated catch block
...
/*
            if(XRVersion.equals(NetconfConstants.XR_VERSION_SIX_ONE)) { 
                transaction.merge(LogicalDatastoreType.CONFIGURATION, InterfaceXrVersionSixDotOneHelper.getInterfaceConfigurationsId(),
                    InterfaceXrVersionSixDotOneHelper.addInterface(port, mtu, setL2Transport));
            }
            */
            if(XRVersion.equals(NetconfConstants.XR_VERSION_SIX_THREE)) { 
                transaction.merge(LogicalDatastoreType.CONFIGURATION, InterfaceXrVersionSixDotThreeHelper.getInterfaceConfigurationsId(),
                    InterfaceXrVersionSixDotThreeHelper.addInterface(port, mtu, setL2Transport));
            }
            /*
            if(XRVersion.equals(NetconfConstants.XR_VERSION_SIX_FIVE)) {
                transaction.merge(LogicalDatastoreType.CONFIGURATION, InterfaceXrVersionSixDotFiveHelper.getInterfaceConfigurationsId(),
                    InterfaceXrVersionSixDotFiveHelper.addInterface(port, mtu, setL2Transport));
            }
            */
            if(XRVersion.equals(NetconfConstants.XR_VERSION_SIX_SIX)) { 
                transaction.merge(LogicalDatastoreType.CONFIGURATION, InterfaceXrVersionSixDotSixHelper.getInterfaceConfigurationsId(),
                    InterfaceXrVersionSixDotSixHelper.addInterface(port, mtu, setL2Transport));
            }
        }
    }
...
/*
            if(XRVersion.equals(NetconfConstants.XR_VERSION_SIX_ONE)) { 
                transaction.merge(LogicalDatastoreType.CONFIGURATION, InterfaceXrVersionSixDotOneHelper.getInterfaceConfigurationsId(),
                    InterfaceXrVersionSixDotOneHelper.addSubInterface(port, mtu));
            }
  */
            
            if(XRVersion.equals(NetconfConstants.XR_VERSION_SIX_THREE)) { 
                transaction.merge(LogicalDatastoreType.CONFIGURATION, InterfaceXrVersionSixDotThreeHelper.getInterfaceConfigurationsId(),
                    InterfaceXrVersionSixDotThreeHelper.addSubInterface(port, mtu));
            }
            /*
            if(XRVersion.equals(NetconfConstants.XR_VERSION_SIX_FIVE)) { 
                transaction.merge(LogicalDatastoreType.CONFIGURATION, InterfaceXrVersionSixDotFiveHelper.getInterfaceConfigurationsId(),
                    InterfaceXrVersionSixDotFiveHelper.addSubInterface(port, mtu));
            }
            */
            if(XRVersion.equals(NetconfConstants.XR_VERSION_SIX_SIX)) { 
                transaction.merge(LogicalDatastoreType.CONFIGURATION, InterfaceXrVersionSixDotSixHelper.getInterfaceConfigurationsId(),
                    InterfaceXrVersionSixDotSixHelper.addSubInterface(port, mtu));
            }
        }
    }
