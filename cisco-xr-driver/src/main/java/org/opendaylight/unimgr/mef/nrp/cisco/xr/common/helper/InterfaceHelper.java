...
import org.opendaylight.unimgr.mef.nrp.cisco.xr.version.six.five.InterfaceIdentifierRev170907;
import org.opendaylight.unimgr.mef.nrp.cisco.xr.version.six.one.InterfaceIdentifierRev150730;
...
//    }
        ...
        return nodes.stream().flatMap(cn -> {
            ...
            try {
                ...
                if (mountPoint.isPresent()) {
                      ...
                      try {
                            if(InterfaceIdentifierRev150730.ports(tx).count() > 0) {
                                nodeInterfaceIdentifierList.put(key.getNodeId().getValue(), InterfaceIdentifierRev150730.getInterfaceConfigurationsId().getTargetType().getPackageName());
                                return InterfaceIdentifierRev150730.toTp(nodes, mountService, mountIds).stream();
                           }
                          }
                      catch (Exception e) {
                      ...
                      try {
						 if (InterfaceIdentifierRev170907.ports(tx).count() > 0) { 
							 nodeInterfaceIdentifierList.put(key.getNodeId().getValue(), InterfaceIdentifierRev170907.getInterfaceConfigurationsId().getTargetType().getPackageName());
							 return InterfaceIdentifierRev170907.toTp(nodes, mountService, mountIds).stream(); }
                         }
                      catch (Exception e) {
                            // TODO Auto-generated catch block
                         }
    ...
    public static void setInterfaceConfiguration(ServicePort port, long mtu, boolean setL2Transport, WriteTransaction transaction) {
    	if( nodeInterfaceIdentifierList.containsKey(port.getNode().getValue()) ) {
    		...
    		if(identifierPath.contains("rev150730")) { 
	    		transaction.merge(LogicalDatastoreType.CONFIGURATION, InterfaceIdentifierRev150730.getInterfaceConfigurationsId(),
	    				InterfaceIdentifierRev150730.addInterface(port, mtu, setL2Transport));
    		}
    		
    		if(identifierPath.contains("rev170907")) { 
	    		transaction.merge(LogicalDatastoreType.CONFIGURATION, InterfaceIdentifierRev170907.getInterfaceConfigurationsId(),
	    				InterfaceIdentifierRev170907.addInterface(port, mtu, setL2Transport));
    		}
    	}
    }
    ...
    public static void setSubInterfaceConfiguration(ServicePort port, WriteTransaction transaction, long mtu) {
    	if( nodeInterfaceIdentifierList.containsKey(port.getNode().getValue()) ) {
    		...
    		if(identifierPath.contains("rev150730")) { 
    			transaction.merge(LogicalDatastoreType.CONFIGURATION, InterfaceIdentifierRev150730.getInterfaceConfigurationsId(),
	    				InterfaceIdentifierRev150730.addSubInterface(port, mtu));
    		}
    		if(identifierPath.contains("rev170907")) { 
    			transaction.merge(LogicalDatastoreType.CONFIGURATION, InterfaceIdentifierRev170907.getInterfaceConfigurationsId(),
	    				InterfaceIdentifierRev170907.addSubInterface(port, mtu));
    		}
        }
    }
